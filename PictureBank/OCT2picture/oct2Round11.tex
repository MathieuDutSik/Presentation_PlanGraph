...
bounded by $q$ arcs (paths of edges) belonging to central circuits 
(different or coinciding), such that all $q$ arcs form together a circle. 
A patch can be seen as a $q$-gon; we admit also 
...
%obtained when two central circuits intersect in this vertex.  
...
$c(A)=\sum_{k\geq 1} (4-k)p'_k$. So, the $k$-gon can be seen as,
...
(ii) Any patch $A$ is the union of regular patches $A_1$, \dots, $A_p$; one has  $c(A)=(4-q_1)+\dots+(4-q_p)$, 
!!where each patch $A_i$ is bounded by $q_i$ arcs.!!
...
$s=1$ and $s'=1$. So, one has, say, $e_1\cap e'_1=\{w_1\}$ and 
...
\centering
\mbox{
\subfigure[The case $s=1$, $s'=1$]{\epsfig{figure=Case3-3.eps,width=.25\textwidth}}
\subfigure[The case $s=2$, $s'=2$]{\epsfig{figure=Case2-2.eps,width=.25\textwidth}}
\subfigure[The case $s=2$, $s'=1$]{\epsfig{figure=InfiniteStructure.eps,width=.30\textwidth}}
}
...
(i) an $n$-vertex $4$-hedrites exists if and only if $n\geq 2$, even.

(ii) an $n$-vertex $5$-hedrites exists if and only if $n\geq 3$, $n\not= 4$.

(iii) an $n$-vertex $6$-hedrites exists if and only if $n\geq 4$.

(iv) an $n$-vertex $7$-hedrites exists if and only if $n\geq 7$.

(v) an $n$-vertex $8$-hedrites exists if and only if $n\geq 6$, $n\not= 7$.
...
We write $\sim$ if the projection in the pictures and Table below 
is different from the one given in corresponding cases above.
