...
an $n$-vertex!! $i$-hedrite has $p_4=n+2-i$ $4$-gons and $p_2+p_3=i$ other faces.
Clearly, $(i;p_2,p_3)=(8;0,8)$, !! (7;1,6)$,
$(6;2,4)$, $(5;3,2)$ and $(4;4,0)$ are all possibilities. 
...
\item[(v)] For $8$-hedrites: all known symmetry groups are $C_{1}$, $C_s$, $C_2$, $C_{2v}$, $C_i$, $C_{2h}$, $D_2$, $D_{2d}$, $D_{2h}$, $D_3$, $D_{3d}$, $D_{3h}$, $D_4$, $D_{4d}$, $D_{4h}$, $O$, $O_h$.
\end{itemize}
...
% for $4_n$ the symmetry groups are  $C_{1}$, $C_s$, $C_2$, $C_{2v}$, $C_{2h}$, $D_2$, $D_{2d}$, $D_{2h}$, $D_3$, $D_{3d}$, $D_{3h}$, $D_6$, $D_{6h}$, $C_i$, $O$, $O_h$ 
% for $3_n$ the symmetry groups are $D_2$, $D_{2d}$, $D_{2h}$, $T$, $T_d$.
...
%call this edge-partition {\it CC-partition}.!! 
...
%\begin{claim}
%For any $t \ge 1$ there exist an octahedrite $P_t$ with $n=8t-2$ vertices, having two
%simple central circuits of length $4t$ each and exactly $2t$ common vertices.
%The octahedrites $P_1, P_2$ are Nos. 6, 14-1. The triangles of $P_t, t>1$, are organized into four
%isolated pairs of adjacent ones.
%
%If $t$ is even, then remaining central circuits are $C_3,C_4$, having 
%$Int(C_3)=Int(C_4)=(\frac{t}{2}-1;t^3)$.
%
% If $t$ is odd, then only one central circuit $C_3$ remains; it has
%$Int(C)=(2t-2; (2t)^2)$.
%\end{claim}
%
%In fact, Claim 1 can be verified by inspection of Figure 1.
%
%
%
%Any $APrism_{m}^{k}$ (defined in Section 5 below) with gcd(m,k+2)=2 has two central circuits of length
%$m(k+1)$ with $\frac{mk}{4}$ self-intersections each and intersecting in
%$\frac{m(k+2)}{2}$ vertices. In particular, two central 20-circuits of
%$APrism_{4}^{4}=oc_{20}$ intersect in 12 vertices, i.e. in $\frac{3}{5}$ of
%all of them.
% it SEEMS TO BE maximum for octahedrites. 
...
%, or one of the four $8$-hedrite with $20$, $22$, $30$, $32$ given on the Figu%re XXXXXXXX.
!!perhaps, better put!!
...
%the projections of links $6^3_1$, $6^3_2$, $8^3_6$. 
